...
class ModelsTests(TestCase):
    ...
    def test_author_str(self):
        ...
        self.assertEqual(
            str(author), f"{author.username} ({author.first_name} {author.last_name})"
        )

    def test_book_str(self):
        format_ = LiteraryFormat.objects.create(name="test")
        book = Book.objects.create(title="Test", price="10.15", format=format_)

        self.assertEqual(
            str(book), f"{book.title} (price: {book.price}, format: {format_.name})"
        )
